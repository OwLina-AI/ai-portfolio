...
document.addEventListener('click',e=>{const sh=e.target.closest('.shot');if(sh)openLbFromShot(sh);if(e.target.matches('.lb-close')){e.preventDefault();e.stopPropagation();closeLightbox()}if(e.target.matches('.lightbox'))closeLightbox();if(e.target.matches('.lb-prev')&&lbItems.length){e.preventDefault();e.stopPropagation();lbIndex=(lbIndex-1+lbItems.length)%lbItems.length;showLbItem()}if(e.target.matches('.lb-next')&&lbItems.length){e.preventDefault();e.stopPropagation();lbIndex=(lbIndex+1)%lbItems.length;showLbItem()}});
document.addEventListener('touchend',e=>{if(e.target.matches('.lb-close')){e.preventDefault();e.stopPropagation();closeLightbox()}},{passive:false});
